...
(function() {
    ...
    function setupCanvas() {
        ...
        [colorCanvas.width, colorCanvas.height, maskCanvas.width, maskCanvas.height] = [width, height, width, height];
        [colorCanvas.height, maskCanvas.height] = [height, height];

        // 绘制边界到掩膜
        maskCtx.clearRect(0, 0, width, height);
    maskCtx.strokeStyle = '#000';
    // 线宽随分辨率放大，尽量封闭缝隙，防止一个种子淹没全图
    const lineWidth = Math.max(width, height) * 0.012; // 约 1.2% 的短边
    ...
    function renderColors() {
        ...
        for (let i = 0; i < src.length; i += 4) {
            ...
            dst[i] = r; dst[i + 1] = g; dst[i + 2] = b; dst[i + 3] = 200; // 半透明
        }
